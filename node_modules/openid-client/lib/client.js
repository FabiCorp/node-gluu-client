...
<body onload="javascript:document.forms[0].submit()">
  ...
  introspectUMA(accessToken, rpt) {
    ...
    headers.Authorization = bearer(token);
    const issuer = this.issuer;
    return this.httpClient.post(issuer.rpt_endpoint + "?token=" + rpt, issuer.httpOptions({
      headers,
    }))
      .then(expectResponse(200))
      .then(response => response.body)
  ...
  authorizationCallback(redirectUri, parameters, checks) {
    ...
    if (params.id_token) {
      promise = Promise.resolve(new TokenSet(params))
        .then(tokenset => this.decryptIdToken(tokenset, 'id_token', "idtoken"))
        .then(tokenset => this.validateIdToken(tokenset, toCheck.nonce, 'authorization', toCheck.max_age, toCheck.state, "idtoken"))
    }

    if (params.code) {
      const grantCall = () => this.grant({
        // grant_type: 'urn:ietf:params:oauth:grant-type:uma-ticket',
        // ticket: '39e216d1-46a2-480e-a569-f5cf80274e57',
        ...
        .then(tokenset => this.decryptIdToken(tokenset, 'id_token', "code"))
        .then(tokenset => this.validateIdToken(tokenset, toCheck.nonce, 'token', toCheck.max_age, "" , "code"))
        .then((tokenset) => {
   ...
   * @api private
   */
  decryptIdToken(token, use, string) {
   ...
   * @api private
   */
  validateIdToken(tokenSet, nonce, returnedBy, maxAge, state, string) {
  ...
  refresh(refreshToken) {
    ...
    return this.grant({
      ...
      .then((tokenset) => {
        ...
        return this.decryptIdToken(tokenset, 'id_token', "refresh")
          .then(() => this.validateIdToken(tokenset, null, 'token', null, "refresh"));
      });
  }
      ...
      .then((response) => {
        if (JWT_CONTENT.exec(response.headers['content-type'])) {
          return Promise.resolve(response.body)
            .then(jwt => this.decryptIdToken(jwt, 'userinfo', 'userinfo'))
            .then((jwt) => {
              if (!this.userinfo_signed_response_alg) return JSON.parse(jwt);
              return this.validateIdToken(jwt, null, 'userinfo', null, "userInfo")
  ...
  //  * @api private
  //  */
  // umaTokenPost(httpOptions) {
  //   return Promise.resolve(this.authFor("token"))
  //     .then((auth) => {
  //       const opts = this.issuer.httpOptions(_.merge(httpOptions, auth, { form: true }));
  //       console.log(opts);
  //       return this.httpClient.post(this.issuer[`${endpoint}_endpoint`], opts);
