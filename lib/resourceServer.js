...
// initialze gluuIssuer for further usage
 ...
 app.get('/resourceSets', (req, res) => {
   if(tokenS.access_token){
     ...
     .then(function (resp) {
       ...
       res.send(core.resourceHtml(config.init.resourceServerAddress +
       ...
       client.introspectUMA(tokenS, rpt).then(function(body) {
         ...
         if(active) {
           ...
           if(scope === "hearthRate") {
             ...
             var rate = core.readFile('measurementData.txt');
             res.send(rate);
             res.end();
           } else {
...
// endpoint for the python script to save the measured hearth rate
 app.post('/measurementData', (req, res) => {
   var measurement = req.body.data;
   core.saveToFile(dataPath, measurement);
   res.end();
 });
